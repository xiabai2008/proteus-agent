...
# 预置技能种子（web 侦察的两条经验；幂等，写进模式记忆分区）
...
# DSH 宿主会话审计桥：宿主侧每次工具调用（含目标动作裁决）并入独立证据链
...
python examples/benchmark.py --suite dsh-session  # 判定 DSH 宿主会话（读宿主桥证据链，离线）
...
模式层 · ModeProfile（一等公民，本次新建的核心）
  persona / capability / permission / budget / scope / verifier / skills / sandbox
        │
内核层 · XPentest（已 fork 入仓 `penagent/` 包 · 430 测试）
...
**测试**：430 passed / 14 skipped（本机 torch 2.8.0+cpu 就位；torch 保持可选依赖定位，屏蔽时 RL 链路用例按环境跳过；skip 全是容器类——Docker 暂停或沙箱镜像未构建）。
