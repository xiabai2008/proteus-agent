...
python examples/benchmark.py --suite all          # ctf 33 题 + 渗透侦察 + 07 靶场
...
模式层 · ModeProfile（一等公民，本次新建的核心）
  persona / capability / permission / budget / scope / verifier / skills / sandbox
        │
内核层 · XPentest（已 fork 入仓 `penagent/` 包 · 287 测试）
...
**测试**：287 passed（本机 torch 2.8.0+cpu 就位；torch 保持可选依赖定位，屏蔽时 RL 链路用例按环境跳过）。
